...
class BaseDataModule(LightningDataModule):
    ...
    def load_data(self, split) -> None:
        ...
        setattr(
            self,
            f"{split}_save_data_path",
            osp.join(
                "assets",
                "datasets",
                f"{split}_{self.config.dataset_name}-{self.config.subset}",
            ),
        )
        ...
        if not osp.exists(name_file_path) or self.config.create_dataset:
            ...
            setattr(
                self,
                name_dataset,
                getattr(self, name_dataset).cast_column(
            ...
            self._save_dataset(
                split, name_file_path, metadata_artifact, f"{split} dataset"
            )
        else:
            file = open(name_file_path, "rb")
            setattr(self, name_dataset, pickle.load(file))
    ...
    def process_dataset(self, split, processor, batch_size=512):
        ...
        if not osp.exists(name_file_path) or self.config.create_dataset:
            ...
            self._save_dataset(
                split, name_file_path, metadata_artifact, f"{split} dataset processed"
            )
        else:
            self.logger.info(
                f"{split} dataset already exists no processing necessary ..."
            )

            file = open(name_file_path, "rb")
            setattr(self, name_dataset, pickle.load(file))
            self.logger.info(f"Loaded processed {split} dataset : {name_file_path}")
    ...
    def filtered_data(self, split, top_db=15) -> None:
        ...
        if not osp.exists(name_file_path) or self.config.create_dataset:
            ...
            setattr(
                ...
                name_dataset,
                getattr(self, name_dataset).map(
                    lambda x: {"audio": trim(np.array(x["audio"]), top_db=top_db)[0]},
                    num_proc=self.config.num_proc,
                    load_from_cache_file=False,
                ),
            )
            setattr(
                self,
                name_dataset,
                getattr(self, name_dataset).filter(
                    lambda x: len(x["audio"])
                    < self.config.max_input_length_in_sec * self.sampling_rate,
                    num_proc=self.config.num_proc,
                    load_from_cache_file=False,
                ),
            )
            ...
            self._save_dataset(
                ...
                name_file_path,
                metadata_artifact,
                f"{split} dataset processed and filtered",
            )

        else:
            file = open(name_file_path, "rb")
            setattr(self, name_dataset, pickle.load(file))
            self.logger.info(f"Loaded filtered {split} dataset : {name_file_path}")
    ...
    def create_phonemes(self, split) -> None:
        ...
        language = (
            self.config.language[:2] if self.config.language[:2] != "zh" else "cmn"
        )
        ...
        setattr(
            self,
            name_dataset,
            getattr(self, name_dataset).add_column(
                "phonemes",
                backend.phonemize(
                    getattr(self, name_dataset)["sentence"],
                    njobs=self.config.num_proc,
                    separator=separator,
                ),
            ),
        )
    ...
    def _save_dataset(
        self, split, name_file_path, metadata_artifact, description_artifact
    ):
        ...
        self.push_artefact(name_file_path, metadata_artifact, description_artifact)
